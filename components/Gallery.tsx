    ...
    <div className="animate-fadeIn bg-white min-h-screen">
       ...
       <section className="py-12 md:py-20 px-4 md:px-8 max-w-[1800px] mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 mb-8 md:mb-12">
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 sm:px-6 sm:py-2.5 rounded-full font-bold text-xs sm:text-sm md:text-base transition-all duration-500 ease-out transform hover:-translate-y-1 transform-gpu will-change-transform antialiased [backface-visibility:hidden] ${
                  activeCategory === cat
          ...
          {filteredImages.length === 0 ? (
            <div className={`py-16 sm:py-24 text-center bg-gray-50/60 rounded-3xl border border-dashed border-gray-200 my-8 transition-all duration-700 ease-out ${
              ...
              <button
                ...
                className="px-6 py-3 bg-[#E11D48] hover:bg-rose-600 text-white font-bold text-sm rounded-xl shadow-md hover:shadow-lg transition-all duration-500 ease-out active:scale-95 inline-flex items-center gap-2"
              >
                <i className="fa-solid fa-border-all"></i> View All Photos
              </button>
            </div>
          ) : (
            /* Masonry Grid with Silky Smooth 700ms Transition */
            <div className={`columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4 sm:gap-6 space-y-4 sm:space-y-6 transition-all duration-700 ease-out transform-gpu will-change-transform ${
              isAnimating ? 'opacity-0 scale-95 translate-y-3' : 'opacity-100 scale-100 translate-y-0'
            }`}>
               {filteredImages.map((img, idx) => (
                  <div key={`${img.src}-${idx}`} className="break-inside-avoid">
                     <button 
                       ...
                       className="block w-full text-left relative group rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-700 ease-out hover:-translate-y-2 bg-gray-100 transform-gpu will-change-transform antialiased [backface-visibility:hidden]"
                     >
                        <img 
                          src={img.src} 
                          alt={img.alt} 
