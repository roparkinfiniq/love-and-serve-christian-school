    ...
    <section className="py-20 px-4 sm:px-6 bg-gradient-to-b from-white via-rose-50/30 to-white overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative mb-6 sm:mb-10">
          ...
          <div className="flex md:hidden items-center justify-center gap-1.5 text-[11px] font-bold text-rose-500 mb-2">
            <i className="fa-solid fa-arrows-left-right text-[10px] animate-pulse"></i>
        ...
        <div className="relative bg-white border border-gray-100 rounded-2xl sm:rounded-3xl p-5 sm:p-8 md:p-12 shadow-lg hover:shadow-2xl transition-all duration-700 ease-in-out overflow-hidden transform-gpu will-change-transform antialiased [backface-visibility:hidden]">
          {/* Watermark Year Background */}
          <div className={`absolute right-3 bottom-1 text-6xl sm:text-9xl font-black text-rose-500/5 select-none pointer-events-none font-mono tracking-tighter transition-all duration-500 ease-in-out transform-gpu ${
            isAnimating ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'
          }`}>
            {activeMilestone.year}
          </div>

          <div className={`relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 md:gap-10 transition-all duration-500 ease-in-out transform-gpu will-change-transform antialiased [backface-visibility:hidden] ${
            ...
            <div className="flex-1 space-y-3 sm:space-y-4 text-left">
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                <span className="bg-rose-100 text-[#E11D48] font-black text-lg sm:text-2xl px-3 sm:px-4 py-1 rounded-xl shadow-xs">
                  {activeMilestone.year}
                </span>
                <span className="text-[11px] sm:text-xs font-bold text-gray-400 uppercase tracking-widest">
                  Milestone {activeIndex + 1} of {HISTORY_DATA.length}
                </span>
              </div>

              <h3 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-gray-900 leading-snug">
                {activeMilestone.title}
              </h3>

              <p className="text-gray-600 text-xs sm:text-sm md:text-base leading-relaxed max-w-2xl font-normal">
                {activeMilestone.description}
              </p>
            </div>

            {/* Right Navigation & Control Column */}
            <div className="flex items-center gap-3 self-end md:self-center shrink-0 pt-3 md:pt-0 border-t md:border-t-0 border-gray-100 w-full md:w-auto justify-between md:justify-end">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={handlePrev}
                  className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gray-100 hover:bg-[#E11D48] text-gray-700 hover:text-white transition-all duration-500 ease-in-out flex items-center justify-center shadow-xs active:scale-95"
                ...
                <button
                  onClick={handleNext}
                  className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-[#E11D48] hover:bg-rose-600 text-white transition-all duration-500 ease-in-out flex items-center justify-center shadow-md hover:shadow-lg active:scale-95"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5 sm:gap-3 mt-6 sm:mt-8">
          {HISTORY_DATA.map((item, idx) => (
            <button
              key={item.year}
              onClick={() => handleSelectIndex(idx)}
              className={`p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border text-left transition-all duration-500 ease-in-out ${
                idx === activeIndex 
                  ? 'bg-rose-50 border-[#E11D48] text-[#E11D48] shadow-xs font-bold ring-1 ring-[#E11D48]' 
