...
const MessengerWidget: React.FC = () => {
  ...
  const [chatMessages, setChatMessages] = useState<Message[]>([
    ...
    {
      sender: 'bot',
      text: 'You can type any question below or select a quick topic. We are here to assist you!',
      time: 'Just now'
    }
  ]);
  ...
  const quickTopics = [
    ...
    { label: '📞 Contact Officer', text: 'Please connect me with an Admissions Officer.' }
  ];

  const getAutomatedResponse = (text: string): string => {
    const lower = text.toLowerCase();
    if (lower.includes('admission') || lower.includes('enroll') || lower.includes('apply')) {
      return "Thank you for your interest! SY 2026-2027 Admissions for Preschool, Elementary, and Junior High are currently open. You can apply on our Admissions page or leave your phone number/email here for our team to contact you!";
    }
    if (lower.includes('tuition') || lower.includes('fee') || lower.includes('cost') || lower.includes('scholarship')) {
      return "Our tuition fee schedule varies by grade level. Please leave your email or contact number, and our admissions office will send you the detailed fee breakdown.";
    }
    ...
    if (lower.includes('contact') || lower.includes('phone') || lower.includes('call') || lower.includes('number')) {
      return "You can call us directly at +63999-982-1836 or +63917-710-7075. You can also leave your contact info right here!";
    }
    ...
    <div className="fixed bottom-5 right-5 z-[100] flex flex-col items-end">
        ...
        <div className="mb-4 w-[340px] sm:w-[380px] h-[530px] bg-white rounded-3xl shadow-2xl border border-rose-100 flex flex-col overflow-hidden animate-fadeIn duration-300 transform-gpu">
          ...
          <div className="p-3 bg-white border-t border-gray-100 shrink-0">
            ...
            <div className="flex items-center justify-between text-[10px] text-gray-400 pt-2 px-1">
              ...
              >
                <span>Or open in Facebook Messenger</span>
      ...
      {showTooltip && !isOpen && (
        ...
        >
          ...
          <span className="text-gray-800 tracking-tight">Need help? Live Chat with us!</span>
          <button 
            type="button"
            onClick={(e) => {
              e.stopPropagation();
